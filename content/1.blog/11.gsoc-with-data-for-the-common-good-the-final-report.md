...
Everything in that paragraph landed except the last word. Containerization is written and in review
rather than merged, and I would rather say so here than leave a reader to notice it later. The rest
of it looks like this:

| Before | After |
| --- | --- |
| 3 Python files, 435 lines, no tests | 27 modules, about 4,900 lines, with about 5,900 lines of tests |
...
Ten endpoints are documented and served on the merged branch, seven current and three kept alive and
marked deprecated so the frontend can migrate on its own schedule. The endpoint check still in
review makes eleven, and is included in the documentation below.

![The generated OpenAPI documentation, showing the authorization, patients and providers endpoint groups, including the endpoint check that is still in review.](/blog-images/gsoc-final/report-05-openapi.png){style="display: block; margin: 2rem auto; max-width: 100%; border-radius: 8px;"}

The suite runs 215 tests offline on the merged branch, in about a minute, and 306 once the
endpoint-check work in review lands, alongside a further 18 checks marked `live` that are excluded
by default and hit real servers when they are asked for. It is built around end-to-end flows rather
than isolated method calls: a test drives discovery, then the authorization URL, then the token
exchange, then a read, and asserts on what the caller ends up seeing.

Two servers are verified live from end to end, and I walked the whole flow again while writing this.
A patient connects, logs in at the provider's own page, approves, and their records come back.
...
rather than PKCE alone, chosen by the adapter from Epic's own discovery document. Oracle Health's
sandbox is registered, configured, and covered by a discovery document captured from the live
tenant, and completing a live login there needs one credential that is still outstanding.
...
Three pieces are unfinished. I would rather describe each precisely enough that somebody else could
pick it up than list their titles and move on.

**Containerization.** A `Dockerfile` and a Compose file, so that `docker compose up` on a clean
checkout serves the documentation with no step beyond copying the example environment file. The
shape is a multi-stage build that installs from the lock file in one stage and ships a slim runtime
running as a non-root user, a pair of Compose services with a healthcheck on the database and the
application waiting for it, and `alembic upgrade head` on container start so a fresh volume comes up
already migrated. The published database port has to be configurable, because 5432 is frequently
already taken on a developer's machine. This is written and in review rather than merged.
...
| [#29](https://github.com/chicagopcdc/smart-on-fhir-backend/pull/29) | Check an endpoint is usable before OAuth starts | Open, in review |
| [#22](https://github.com/chicagopcdc/smart-on-fhir-backend/issues/22) | Containerize the backend with Docker Compose | Open |
...
explicitly. The last commit inside Google Summer of Code 2026 is the final commit on the
containerization branch, made in August 2026, and every pull request listed above is program work.
Anything landing after that point is not part of this submission.
